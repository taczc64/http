...
import (
	...
	"github.com/taczc64/http/httputil"
	"net/url"
	"os"
	"reflect"
	"runtime"
...
// transport_test.go (for HTTP/1) or x/github.com/taczc64/http2/transport_test.go
// (for HTTP/2).
...
func testWriteHeaderAfterWrite(t *testing.T, h2, hijack bool) {
	...
	wantLog := "http: superfluous response.WriteHeader call from github.com/taczc64/http_test.testWriteHeaderAfterWrite.func1 (clientserver_test.go:"
	if hijack {
		wantLog = "http: response.WriteHeader on hijacked connection from github.com/taczc64/http_test.testWriteHeaderAfterWrite.func1 (clientserver_test.go:"
	}
