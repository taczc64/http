...
import (
	...
	"log"
	"net"
	"github.com/taczc64/http/httptrace"
	"net/textproto"
	"net/url"
	"os"
	"reflect"
...
// github.com/taczc64/http against an *http2.Transport that's either bundled into
// h2_bundle.go or supplied by the user via x/github.com/taczc64/http2.
//
...
func (t *Transport) onceSetNextProtoDefaults() {
	...
	//
	// TODO: also add this to x/github.com/taczc64/http2.Configure Transport, behind
...
func (t *Transport) roundTrip(req *Request) (*Response, error) {
	...
	if isHTTP {
		for k, vv := range req.Header {
			if !httpguts.ValidHeaderFieldName(k) {
				return nil, fmt.Errorf("github.com/taczc64/http: invalid header field name %q", k)
			}
			for _, v := range vv {
				if !httpguts.ValidHeaderFieldValue(v) {
					return nil, fmt.Errorf("github.com/taczc64/http: invalid header field value %q for key %v", v, k)
				}
			}
		}
	}
	...
	if req.Method != "" && !validMethod(req.Method) {
		return nil, fmt.Errorf("github.com/taczc64/http: invalid method %q", req.Method)
	}
	if req.URL.Host == "" {
		req.closeBody()
...
var ErrSkipAltProtocol = errors.New("github.com/taczc64/http: skip alternate protocol")
...
func (e transportReadFromServerError) Error() string {
	return fmt.Sprintf("github.com/taczc64/http: Transport failed to read from server: %v", e.err)
}
...
func (t *Transport) dial(ctx context.Context, network, addr string) (net.Conn, error) {
	...
	if t.Dial != nil {
		...
		if c == nil && err == nil {
			err = errors.New("github.com/taczc64/http: Transport.Dial hook returned (nil, nil)")
		}
		return c, err
	}
...
func (w *wantConn) tryDeliver(pc *persistConn, err error) bool {
	...
	w.err = err
	if w.pc == nil && w.err == nil {
		panic("github.com/taczc64/http: internal error: misuse of tryDeliver")
	}
	close(w.ready)
	return true
}
...
func (t *Transport) decConnsPerHost(key connectMethodKey) {
	...
	if n == 0 {
		...
		panic("github.com/taczc64/http: internal error: connCount underflow")
	}
...
func (t *Transport) dialConn(ctx context.Context, cm connectMethod) (pconn *persistConn, err error) {
	...
	if cm.scheme() == "https" && t.DialTLS != nil {
		...
		if pconn.conn == nil {
			return nil, wrapErr(errors.New("github.com/taczc64/http: Transport.DialTLS returned (nil, nil)"))
		}
...
func (pc *persistConn) mapRoundTripError(req *transportRequest, startBytesWritten int64, err error) error {
	...
	if pc.isBroken() {
		...
		return fmt.Errorf("github.com/taczc64/http: HTTP/1.x transport connection broken: %v", err)
	}
	return err
}
...
func (pc *persistConn) readLoop() {
	...
	for alive {
		...
		if err != nil {
			if pc.readLimit <= 0 {
				err = fmt.Errorf("github.com/taczc64/http: server response headers exceeded %d bytes; aborted", pc.maxHeaderResponseSize())
			}
	...
	for {
		...
		if is1xxNonTerminal {
			...
			if num1xx > max1xxResponses {
				return nil, errors.New("github.com/taczc64/http: too many 1xx informational responses")
			}
...
var errTimeout error = &httpError{err: "github.com/taczc64/http: timeout awaiting response headers", timeout: true}

// errRequestCanceled is set to be identical to the one from h2 to facilitate
// testing.
var errRequestCanceled = http2errRequestCanceled
var errRequestCanceledConn = errors.New("github.com/taczc64/http: request canceled while waiting for connection") // TODO: unify?
...
func (tlsHandshakeTimeoutError) Error() string   { return "github.com/taczc64/http: TLS handshake timeout" }
